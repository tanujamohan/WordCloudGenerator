<?php
    ...
    function printLyrics($artist, $title){
        ...
        $source="";
        $url="";
        $filename = $title . "-" . $artist;//get_parameter("filename");
       ...
       // print trim(strip_tags(html_entity_decode($lyrics)));
        return trim(strip_tags(html_entity_decode($lyrics)));
    }
    ...
    $text = printLyrics("Taylor Swift" , "Love Story");
    $words = str_replace("'", '', $text);
    echo "\n\n\n";
    echo "Hello\n";
    printLyrics("Bastille" , "Bad Blood");
    echo "\n\n\n";
    printLyrics("Katy Perry" , "Fireworks");



//$words2 = preg_split("/(?<=\w)\b\s*[!?.,]*/", $words, -1, PREG_SPLIT_NO_EMPTY);

$words2 = preg_split("/(?<=\w)\b\s*[!?.,-;]*/", $words, -1, PREG_SPLIT_NO_EMPTY);
$trimmed_array=array_map('trim',$words2);

//FINAL STRUCTURE: FOR TANUJA
$words3 = array_count_values($trimmed_array);



//unset($words3[array_search('A', array_keys($words3))]); 
//unset($words3[1]);
//$result = array_flip($words3);
arsort($words3);
print_r($words3);
$index_a= array_search("a",array_keys($words3));
$index_the = array_search("the", array_keys($words3));



//$index_a = $words3['a'];
//$index_the = $words3['the'];

//$index = $result[array_search('a', array_keys($result))];

array_splice($words3, $index_a, 1);
array_splice($words3, $index_the, 1);
//print_r($words3);
